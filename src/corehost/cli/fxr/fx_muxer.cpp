    ...
    pal::string_t* impl_dir)
{
    ...
    if (mode == host_mode_t::muxer && !is_framework_dependent)
    {
        ...
        else if (get_app(fx_definitions).get_name().empty()) //todo: better way to verify this?
        {
...
)
{
    ...
    for (pal::string_t dir : hive_dir)
    {
        ...
        if (!do_roll_forward)
        {
            trace::verbose(_X("Did not roll forward because patch_roll_fwd=%d, roll_fwd_on_no_candidate_fx=%d, chose [%s]"),
                *(fx_ref.get_patch_roll_fwd()), *(fx_ref.get_roll_fwd_on_no_candidate_fx()), fx_ver.c_str());
    ...
    int32_t* required_buffer_size)
{
    ...
    if (is_framework_dependent)
    {
        ...
        else
        {
            ...
            // Read the shared frameworks; retry is necessary when a framework is already  resolved, but then a newer version is processed.
            int rc = 0;
            int retry_count = 0;
            do
            {
